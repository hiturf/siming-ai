...
from .models import OperationEvent, OperationRun
...
def ensure_operation(
    ...
    *,
    ...
    operation.progress_current = (
        progress_current if progress_current is not None else operation.progress_current
    )
    operation.progress_total = (
        progress_total if progress_total is not None else operation.progress_total
    )
    operation.input_revision = (
        input_revision if input_revision is not None else operation.input_revision
    )
...
def update_operation(
    ...
    *,
    ...
    if (
        activity
        or (event_type is not None and event_type not in health_only_events)
        or output
        or checkpoint
    ):
    ...
    *,
    ...
    def _record(session: Session) -> None:
        ...
        health = (
            ...
            if signal in {"quiet", "suspected_stall", "stalled", "disconnected"}
            else "active"
        )
        event_type = (
            signal
        ...
        update_operation(
            ...
            event_type=event_type,
            payload=payload,
            progress_current=(payload or {}).get("progress_current")
            if isinstance(payload, dict)
            else None,
            progress_total=(payload or {}).get("progress_total")
            if isinstance(payload, dict)
            else None,
...
def fail_operation(
    ...
    *,
    ...
    _write_with_uow(_fail, db)


def register_operation_actions(operation_id: str, **handlers: Callable[[], Any]) -> None:
    _ACTION_HANDLERS[operation_id] = {
        name: handler for name, handler in handlers.items() if callable(handler)
    }
...
def _derived_health(operation: OperationRun, now: datetime) -> str:
    return derive_health(
        ...
        last_activity_at=operation.last_activity_at,
        last_output_at=operation.last_output_at,
...
def serialize_operation(operation: OperationRun, *, include_events: bool = False) -> dict[str, Any]:
    ...
    if operation.progress_mode == "determinate" and operation.progress_total:
        percent = max(
            0, min(100, round((operation.progress_current or 0) / operation.progress_total * 100))
        )
    public_result = None
    if result:
        public_result = {
    ...
    data = {
        ...
        or (
            ...
            if status in TERMINAL_STATUSES or status == "waiting_user"
            else None
        ),
